...
export function useBulkPaymentLogic() {
  ...
  const [reportStats, setReportStats] = useState<{
    ...
    holdAddItems: { month: string; biz: string; reason: string; amount: number; type: 'HOLD' | 'ADD' | 'CANCEL' | 'BONUS' }[];
  ...
  const generateAllSummaryText = useCallback(() => {
    const buDetailsText = [
      ...
      .map((bu) => `- ${bu.name}: ${formatMoneyVND(bu.value).replace(" ₫", "")} VND`)
      .join("\n");

    const sheet1Text = Object.entries(dynamicReportStats?.sheet1Totals || {})
      .map(([biz, amount]) => `- ${biz}: ${formatMoneyVND(amount).replace(" ₫", "")} VND`)
    ...
    const holdAddText = dynamicReportStats?.holdAddItems && dynamicReportStats.holdAddItems.length > 0
      ? (() => {
          ...
          return Object.values(aggregated)
            .map((item) => `- ${item.biz} [${item.type}]: ${item.amount >= 0 ? "+" : ""}${formatMoneyVND(item.amount).replace(" ₫", "")} VND`)
            .join("\n");
        })()
      : "- Không có khoản Hold/Add/Bonus";
...
Số dòng dữ liệu: ${appData.BankExport.data.length} dòng

I. CHI TIẾT SỐ TIỀN THEO BU:
${buDetailsText}

- BONUS (⏯): ${formatMoneyVND(dynamicReportStats?.bonusTotal || 0).replace(" ₫", "")} VND
- BANK NORTH (AHN+AHP): ${formatMoneyVND(holdPaymentDetails.bankNorthT5).replace(" ₫", "")} VND
- BANK TỈNH (ATH+ATN+APT): ${formatMoneyVND(holdPaymentDetails.bankTinhT5Ae).replace(" ₫", "")} VND

II. CHI PHÍ THÁNG BÁO CÁO (SHEET 1):
${sheet1Text}

III. SỐ TIỀN HOLD / ADD / BONUS THEO MỤC:
${holdAddText}
...
- TỔNG BULK PAYMENT: ${formatMoneyVND(totalBulkPayment).replace(" ₫", "")} VND
- TỔNG TIỀN ACC: ${formatMoneyVND(calculationSummary.calculatedTotal).replace(" ₫", "")} VND
- BONUS (⏯): ${formatMoneyVND(dynamicReportStats?.bonusTotal || 0).replace(" ₫", "")} VND
- HOLD (CÙNG THÁNG): -${formatMoneyVND(dynamicReportStats?.sameMonthHoldTotal || 0).replace(" ₫", "")} VND
- ADD (KHÁC THÁNG): +${formatMoneyVND(dynamicReportStats?.diffMonthAddTotal || 0).replace(" ₫", "")} VND
- TỔNG TIỀN BANK AE: ${formatMoneyVND(calculationSummary.calculatedTotal - calculationSummary.diff).replace(" ₫", "")} VND
- LỆCH ACC & AE: ${formatMoneyVND(calculationSummary.diff).replace(" ₫", "")} VND`;
