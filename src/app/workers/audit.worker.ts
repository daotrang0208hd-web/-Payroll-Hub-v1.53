...
export function runAuditComputation(params: any) {
  ...
  rosterData.forEach((row: any) => {
    ...
    const rawType = String(getVal(row, ["type", "task type", "tk_type", "loại công việc", "code"]) || "").trim();
    ...
    const rid = String(getVal(row, ["id", "id number", "teacher id", "emp id", "mã nv", "manv", "id nv", "mã nhân viên", "staff id", "code", "tk_id"]) || "").trim();
