...
export function calculateTimesheet(params: any) {
  ...
  rosterData.forEach((t: any) => {
    ...
    let rawType = String(getVal(t, ["type", "type code", "type_code", "typecode", "task type", "task", "loại", "loại hoạt động", "event type", "activity", "category", "task type name", "taskType"]) || "").trim();
